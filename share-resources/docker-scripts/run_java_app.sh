...
if [ -r ${SERVICE_HOME}/logback.xml ]; then
  export GENERIC_APP_ARG=--logging.config=${SERVICE_HOME}/logback.xml --spring.profiles.active=
else
  export GENERIC_APP_ARG=--spring.profiles.active=
fi
if [[ "${SERVICE_ENV}" = "gcloud" ]]; then
  ...
  java -jar /app/*.jar --spring.config.location="file:///app/application.yaml,classpath://application-dev.yaml" ${GENERIC_APP_ARG} ${RUN_ENV}
else
