...
if [[ "${SERVICE_ENV}" = "gcloud" ]]; then
  ...
  java -jar /app/*.jar --spring.datasource.url=jdbc:postgresql://host.docker.internal:5432/core_finance_loan_account --spring.config.location="file:///app/application.yaml" ${GENERIC_APP_ARG} ${RUN_ENV}
else
